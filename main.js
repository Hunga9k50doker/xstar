...
class ClientAPI {
  ...
  async checkQuery() {
    return this.makeRequest(`${this.baseURL}/game/showMyQuery`, "post");
  }

  async showInfoQuery(payload) {
    return this.makeRequest(`${this.baseURL}/game/showOneInfoWithAddress`, "post", payload);
  }

  async handleQuery() {
    const { data } = await this.checkQuery();
    if (!data) return;
    let { query } = data;
    while (query.length > 0) {
      await sleep(2);
      await this.showInfoQuery({ address: "0xf702063f779f88c5e10ab00e3ef184879a9ced16", type: null });
      query--;
    }
  }
